...
def ksrnet(input_shape, all_normalized_hpf_list):
    ...
    x1 = layers.Conv2D(30, kernel_size=1, strides=1, padding='same', use_bias=False, name='conv2_par',
                       kernel_regularizer=regularizers.l2(0.0001),
                       kernel_initializer="glorot_uniform")(x1)
    ...
    x1 = tf.keras.layers.Resizing(x.shape[1], x.shape[2])(x1)
    ...
    x2 = layers.Conv2D(32, kernel_size=1, strides=1, padding='same', use_bias=False, name='conv3_par',
                       kernel_regularizer=regularizers.l2(0.0001),
                       kernel_initializer="glorot_uniform")(x2)
    ...
    x2 = tf.keras.layers.Resizing(x.shape[1], x.shape[2])(x2)
    ...
    x3 = layers.Conv2D(64, kernel_size=1, strides=1, padding='same', use_bias=False, name='conv4_par',
                       kernel_regularizer=regularizers.l2(0.0001),
                       kernel_initializer="glorot_uniform")(x3)
    ...
    x3 = tf.keras.layers.Resizing(x.shape[1], x.shape[2])(x3)
    ...
    model = models.Model(inputs=inp, outputs=out, name='ksrnet')
    return model

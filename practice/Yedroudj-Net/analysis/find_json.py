...
def find_min_loss(dirpath):
    ...
    for p in dirp.glob("*.json"):
        try:
            ...
            continue  # пропустить некорректные/недоступные файлы

        # безопасный доступ к ["logs"]["loss"]
        try:
            loss = data["logs"]["loss"]
        except (TypeError, KeyError):
            continue

        # допускаем числовые строки
        try:
            loss_num = float(loss)
...
def find_max_accuracy(dirpath):
    ...
    for p in dirp.glob("*.json"):
        ...
        # безопасный доступ к ["logs"]["accuracy"]
        try:
...
def find_min_val_loss(dirpath):
    ...
    for p in dirp.glob("*.json"):
        ...
        # безопасный доступ к ["logs"]["val_loss"]
        try:
...
def find_max_val_accuracy(dirpath):
    ...
    for p in dirp.glob("*.json"):
        ...
        # безопасный доступ к ["logs"]["val_accuracy"]
        try:
